...
h(2) = plot_gaussian_ellipsoid(w_exact_vi(st:en), C_exact_vi(st:en,st:en), 1);
...
for m = 1:length(method)
    ...
    h(m+2) = plot_gaussian_ellipsoid(w_all(st:en,t,m), Sigma_all(st:en,st:en,t,m), 1);
...
plot(w_exact_vi(st), w_exact_vi(en), 'o', 'color', 0*[1 1 1], 'linewidth', 8, 'markerfacecolor', 0*[1 1 1], 'markersize', 5);
      t = maxIters;
...
hy = ylabel('\theta_2');
set(gca, 'fontsize', 10);
set([hx,hy], 'fontsize', 10, 'fontname', 'helvetica');
set(hl, 'fontsize', 10, 'fontname', 'helvetica');
%set(gca, 'xtick', [0:5:20], 'ytick', [0:5:10], 'tickdir', 'out');
set(gca, 'xtick', [-30:2:30], 'ytick', [-30:2:30], 'tickdir', 'out');
...
set(map_dot, 'markerSize', 5)
set(map_dot, 'color', 'yellow')

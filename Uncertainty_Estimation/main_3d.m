...
dataset_name = 'fourclass'; %'murphy_synth'; %'breast_cancer_scale'; %'synth'; %'murphy_synth';
...
[n,n]=size(w1);
W=[reshape(w1,n*n,1) reshape(w2,n*n,1) reshape(w1,n*n,1)];
...
W=[reshape(w1,n*n,1) reshape(w2,n*n,1) reshape(w2,n*n,1)];
...
w_all = zeros(D,maxIters,length(method));
Sigma_all = zeros(D,D,maxIters,length(method));
